...
export const searchMemes = (memesRepository: MemesRepository, {searchTerm}: options): MemeThumbnail[] => {
    const query = normalizeSearchTerm(searchTerm)
    if (query.length < 3) {
        throw new Error("The search term should 3 or more characters")
    }
    return memesRepository.searchMemes(query).map(mapMemesDtoToMemesThumbnail);
}
