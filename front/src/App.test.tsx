...
describe('Listado de memes', () => {
  it('should show a list of memes', async () => {
    ...
    for (let element of sliced) {
      const meme = await screen.findByRole('img', {
        name: element.title + '-' + idx,
      })
      expect(meme).toBeInTheDocument()
      expect(meme).toHaveAttribute('src', element.images.original.url)
      idx++
    }
  })
  ...
  it.skip('Should search memes if 3 or more characters are typed', async () => {
    ...
    expect(window.fetch).toBeCalledWith(
      'http://127.0.0.1/api/memes?query=homer',
    )
  })
})
